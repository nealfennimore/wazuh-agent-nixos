...
| `02-build-openssl-with-perl.patch` | The vendored OpenSSL `config` is a `/bin/sh` wrapper that execs `Configure`, and `Configure` carries a `#!/usr/bin/env perl` shebang. The Nix sandbox has no `/usr/bin/env`, so the kernel cannot start it. Calling `perl ./Configure` bypasses shebang resolution and sets the generated Makefile's `PERL` to the same interpreter. | The vendored launcher runs in the sandbox, or upstream invokes `Configure` through an explicit interpreter. |
...
`Configure` through Perl, so editing the unused file had no effect. OpenSSL
3.0.15's `config` wrapper no longer contains the `/usr/bin/env` string that the
old `--replace-warn` targeted either; that rewrite was written for the older
OpenSSL 1.1 launcher.
